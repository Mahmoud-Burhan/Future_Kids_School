            ...
            <div class="card-body">
                ...
                @foreach(preg_split('/(-)/', $data[$counter]->answers) as $index=>$answer)
                    <div class="custom-control custom-radio">
                        ...
                        <label class="custom-control-label" for="customRadio{{$index}}" wire:click="nextQuestion( '{{$data[$counter]->id}}', '{{$data[$counter]->points}}', '{{$answer}}', '{{$data[$counter]->right_answers}}')"> {{$answer}}</label>
                    </div>
                @endforeach
