...
@section('page-header')
    ...
    <div class="page-title">
        <div class="row">
            <div class="col-sm-6">
                <h4 class="mb-0">  {{trans('processing.Add_Title')}}-{{$students->name}}</h4>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb pt-0 pr-0 float-left float-sm-right ">
                    <li class="breadcrumb-item"><a href="#" class="default-color"> {{trans('processing.Dashboard')}}</a>
                    </li>
                    <li class="breadcrumb-item active"> {{trans('processing.Add_Title')}}-{{$students->name}}</li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12 mb-30">
            <div class="card card-statistics h-100">
                <div class="card-body">
                    <form method="post" action="{{route('ProcessingFee.store')}}">
                        @csrf
                        ...
                        <br>
                        <button type="submit" class="btn btn-success">{{trans('processing.Save')}}</button>
                    </form>
                </div>
            </div>
        </div>
