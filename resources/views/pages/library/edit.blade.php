...
@section('page-header')
    ...
    <div class="page-title">
        <div class="row">
            <div class="col-sm-6">
                <h4 class="mb-0"> {{trans('library.Edit_Title')}} - {{$libraries->title}}</h4>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb pt-0 pr-0 float-left float-sm-right ">
                    <li class="breadcrumb-item"><a href="#" class="default-color">{{trans('library.Dashboard')}}</a>
                    </li>
                    <li class="breadcrumb-item active">{{trans('library.Edit_Title')}} - {{$libraries->title}}</li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12 mb-30">
            <div class="card card-statistics h-100">
                <div class="card-body">
                    <form method="post" action="{{route('Library.update','test')}}" enctype="multipart/form-data">
                        @csrf
                        {{method_field('PUT')}}
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title" id="title" class="form-control" required
                                        value="{{$libraries->title}}">
                                    <input type="hidden" name="id" id="id" value="{{$libraries->id}}">
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="grade_id" id="grade_id" class="form-control" style="height: 4%;"
                                        ...
                                        @foreach($grades as $grade)
                                            <option value="{{$grade->id}}" {{ ($grade->id == $libraries->grade_id)?'selected':'' }}>
                                                {{$grade->name}}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-3">
                                <div class="form-group">
                                    ...
                                    <select name="classroom_id" id="classroom_id" class="form-control"
                                            style="height: 4%;"
                                            onchange="console.log($(this).val())" required>
                                            <option value="{{$libraries->classroom_id}}"> {{$libraries->Classroom->class_name}}</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-3">
                                <div class="form-group">
                                    ...
                                    <select name="section_id" id="section_id" class="form-control" style="height: 4%;"
                                            required>
                                        <option value="{{$libraries->section_id}}"> {{$libraries->Section->section_name}}</option>
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="teacher_id" id="teacher_id" class="form-control" style="height: 4%;"
                                        ...
                                        @foreach($teachers as $teacher)
                                            <option value="{{$teacher->id}}" {{ ($teacher->id == $libraries->teacher_id)?'selected':''}}>
                                                {{$teacher->name}}
                                            </option>
                                        @endforeach
                                    </select>
